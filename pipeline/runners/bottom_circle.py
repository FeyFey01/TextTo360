...
def rect_strip_to_disk(strip, radius=None, invert_radial=False):
    """Same as top version, converts a rect strip to a circular disk."""
    h, w = strip.shape[:2]
    if radius is None:
        radius = w // 2
    R = int(radius)
...
def create_mask_from_disk(disk, mask_radius_ratio=0.5):
    """Same as top version, creates alpha mask from circular disk."""
    h, w = disk.shape[:2]
    R = w // 2
...
def bottom_strip_to_circle_and_mask(
    image,
    strip_height=256,
    ...
    """
    Converts the bottom strip of an image to a circular disk and mask.
    Bottom strip is flipped vertically before conversion.
    ...
    # Take bottom strip and flip vertically
    strip = img[h - strip_height : h, :, ...]
    strip_flipped = cv2.flip(strip, 0)
...
# Ensure BGRA
    ...
    # --- SAVE LOGIC ---
    # pipeline\runners\bottom_circle.py -> .. -> .. -> pipeline\
    project_root = Path(__file__).resolve().parent.parent
    save_dir = project_root / "common files" / "Edited Images"
    save_dir.mkdir(parents=True, exist_ok=True)

    # Define paths
    circle_path = save_dir / "bottom_circle.png"
    mask_path = save_dir / "bottom_circle_mask.png"
